...
- **Esforço:** Médio.
- **Dependências:** Recomendado antes de ARCH-013, ARCH-019, ARCH-020, ARCH-022 (todos constroem/consomem esses tipos — fazer isso cedo reduz churn nos refactors maiores). A recomendação era de sequenciamento (reduzir churn), não bloqueio funcional — todos os 4 já concluídos/adiados sem depender disso.
- **Pré-requisitos:** Nenhum.
- **Critérios de Aceite:** N/A — não avaliado, card encerrado antes de codificar.
- **Definition of Done:** N/A.
- **Rollback:** N/A — nenhum código tocado em nenhuma das 2 passagens (S14 original + reabertura).
- **Testes obrigatórios:** N/A.
- **Desfecho (auditoria de 2026-07-18):** reaberto como 1º item do "próximo ciclo de auditoria" recomendado pelo `CP05.md`. Reverificação de premissa (Fase 1) confirmou que nada mudou desde julho. Achado novo (Fase 2): a alternativa proposta em `docs/issues/008` (base com os 3 tipos estendendo) infla `CycleResult` com um campo `error` que ele nunca teve — escopo correto seria só `ToolResult`+`GoalAttempt`. Com esse escopo corrigido, Impacto/Risco/Esforço reavaliados para Baixo/Baixo/Pequeno (abaixo da estimativa original Médio/Médio/Médio). Apesar disso, **decisão do usuário: encerrar sem implementar** — o ganho é antecipatório (facilita um campo futuro hipotético), sem bug ou incidente real motivando, o tipo de mudança que a filosofia do projeto orienta a não perseguir isoladamente. Detalhe completo: `docs/issues/012-arch009-wontfix-decision.md`. Critério de reabertura futura: só se um campo novo de proveniência de tool precisar ser replicado manualmente 2-3 vezes de fato (evidência real, não uma nova auditoria periódica).
- **Métrica que deverá melhorar:** Single Sources.
...
### ARCH-013 — Unificar juiz de sucesso de step ✅ Concluído (2026-07-18, reaberto e implementado após adiamento original da Sprint S21)
...
- **Executado como:** `escalateStepEvalToLLM()` removida inteira (não fundida — eliminada). `StepSemanticValidator` ganhou `shouldPromoteToConfidentSuccess` (contraparte de `shouldDowngradeToPartial`, já existente), usando `GoalStore.promoteLastAttemptToSuccess()` novo (mesmo padrão read-modify-write de `downgradeLastAttemptToPartial()`/`finalizeLastAttemptAsSuccess()`, já testados em produção) para corrigir retroativamente o `GoalAttempt` já persistido. Threshold de promoção = `FAST_PATH_CONFIDENCE_THRESHOLD` (0.72), deliberadamente diferente do threshold de downgrade (0.80) — ver `SPRINTS/S21-ARCH-013.md` para a análise completa de por que essa assimetria evita uma "zona morta" de confiança. Detalhe completo, achados e etapa 4 real (LLM real, goal real, `[SEMANTIC-PROMOTE]` confirmado em log genuíno): `SPRINTS/S21-ARCH-013.md`.
