...
**Status Geral:** 🟢 Em execução — Fase 0, S01-S13/S15/S16/S19/S20/S23/S24, Checkpoints CP01/CP02/CP03 concluídos; S14/S17/S18/S21 adiadas (ver abaixo)
**Progresso:** 19 / 26 ARCH concluídos (~73%) — ARCH-008 (S17), ARCH-009 (S14), ARCH-013 (S21) e ARCH-018 (S18) adiados, não contam como concluídos nem como pendências simples. ARCH-024 concluído (RFC S19 + Impl S23). ARCH-020+ARCH-021 concluídos juntos (S24, ARCH-021 absorvido). ARCH-015 tem RFC aprovada (S20), só conta como concluído quando a Impl (S26) landar — Definition of Done exige as duas etapas
**Sprint Atual:** Nenhuma em andamento (S24 concluída — próxima é 2026-10-S25)
**Próxima Sprint:** 2026-10-S25 (ARCH-019) — decompor `AgentLoop.runWithTools()` (~1793 linhas, o maior método do projeto). ⚠ Maior risco do programa — **NUNCA simultânea com S24** (já concluída, então essa restrição de exclusão mútua deixou de ser um risco de execução; mantida como registro histórico). Mapear TODOS os efeitos colaterais capturados por closure (`cycleHistory`, `usedToolInputs`, `stepCount`) antes de extrair qualquer método; considerar o mesmo padrão de fatiamento em incrementos sequenciais usado em S24, dado que funcionou bem lá (2 checkpoints de etapa 4 em vez de 1, achados de bug pegos por leitura antes de testar).
**Epic Atual:** Epic A concluído; Epic B "concluído" no sentido de nenhum item executável restante nesta ordem — ARCH-005/006/007/010/011 feitos, ARCH-008 adiado, ARCH-012 deliberadamente deferido para o fim do programa; Epic C — ARCH-014/015(RFC)/016/017 feitos/aprovados, ARCH-013/018 adiados; Epic D quase concluído — ARCH-020/021/022/023/024 feitos, só ARCH-019 restante (S25); Epic E concluído — ARCH-025/026 feitos
**Próximo Marco:** 2026-11-CP04
**Última Atualização:** 2026-07-18 (Sprint S24 concluída)
**Build:** 🟢 `npm run build` limpo
**Testes:** 🟢 126/126
**Regressão:** 🟢 126/126 (pós-S24, `S123` novo; `S16` corrigido no Incremento 1, 0 quebras no Incremento 2)
**Riscos Abertos:** 0 materializados — nota permanente: a VPS `venus@10.0.0.10` usada para validação Linux tem histórico de exigir `.env`/`WORKSPACE_DIR` explícito (skill `verify`) para não gerar falso-positivo — ver `docs/issues/002`. S16 (ARCH-005) teve o escopo redesenhado na Fase 1/2 para um fix cirúrgico — risco real ficou bem menor que o estimado, mas isso NÃO deve ser extrapolado para S25 (ver revisão de riscos do CP03) — a redução veio de a premissa do card estar errada, não de a área ser inerentemente mais fácil. S23 também teve risco real baixo (confirmado, não presumido). **S24 concluída com sucesso apesar de ser o maior refactor do programa até agora** — risco mitigado pelo fatiamento em 2 incrementos com etapa 4 própria cada, não pela mudança ser inerentemente mais simples que o estimado (diferente de S16/S23, aqui a premissa de tamanho do card bateu quase exatamente com a medição real). 3 instâncias do mesmo bug de threading de `priorFeedback` foram pegas por leitura cuidadosa do código ANTES de rodar qualquer teste — a suíte de regressão nunca chegou a rodar com o bug presente, então não serve como evidência de que a suíte "teria pegado" sozinha; reforça que decomposição de estado mutável via closure exige revisão humana/LLM linha a linha, não só cobertura de teste. Essa lição — não a redução de risco de S16 — é a que deveria informar S25 (`AgentLoop.runWithTools()`, mesma classe de risco, closures diferentes: `cycleHistory`/`usedToolInputs`/`stepCount`).
...
**Dívida Arquitetural Restante:** 7 ARCH (ARCH-019/S25, ARCH-015-Impl/S26, ARCH-012/S27 — 3 cards executáveis restantes + ARCH-008/ARCH-009/ARCH-013/ARCH-018 adiados) — 26 total − 19 concluídos (S24 fechou ARCH-020+ARCH-021) = 7
