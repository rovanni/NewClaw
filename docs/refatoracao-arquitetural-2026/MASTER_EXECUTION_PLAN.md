...
| 2026-08-S18 | Ago/2026 (reaberto e concluído 2026-07-19) | Decision Ownership | ARCH-018 | 🟢 | S16 | 4171d31 (adiamento original) + implementação (`domainTypes.ts`, `ensureDeliverySuccessCriteria.ts`, `GoalExecutionLoop.ts`, `S30`/`S125.6` corrigidos, `S128` novo) | Reaberto como 4º e último item do próximo ciclo de auditoria; novo `CriterionCheck` (`pending_send_verified_on_disk`) preservando a checagem de disco original, sem reaproveitar `file_exists`; achado real de etapa 4 (replan desnecessário) corrigido e reconfirmado ao vivo com `replans=0`; regressão 131/131 | [ver](SPRINTS/S18-ARCH-018.md) |
...
**Status Geral:** 🟢 **Programa concluído** (2026-07-18), e 🟢 **ciclo de auditoria pós-encerramento também concluído** (2026-07-19). Dos 4 cards formalmente adiados na primeira rodada (ARCH-008/009/013/018), 3 foram implementados na reabertura (ARCH-008, ARCH-013, ARCH-018) e 1 foi encerrado como Won't Fix por decisão explícita (ARCH-009). Todos os 26 cards do backlog têm desfecho final definitivo — nenhum resta em aberto.
**Progresso:** 25 / 26 ARCH concluídos (~96%) — o único não-"concluído" é ARCH-009, encerrado deliberadamente como Won't Fix (`docs/issues/012`), não uma pendência. ARCH-013 (S21), ARCH-008 (S17) e ARCH-018 (S18) reabertos e **implementados** — ver `SPRINTS/S21-ARCH-013.md`/`SPRINTS/S17-ARCH-008.md`/`SPRINTS/S18-ARCH-018.md`. ARCH-024 concluído (RFC S19 + Impl S23). ARCH-020+ARCH-021 concluídos juntos (S24, ARCH-021 absorvido). ARCH-019 concluído (S25, 6 incrementos). ARCH-015 concluído (RFC S20 + Impl S26). ARCH-012 concluído (RFC + Impl de escopo reformulado, S27).
**Sprint Atual:** Nenhuma em andamento — S18/ARCH-018 concluída, último item do ciclo de auditoria pós-encerramento.
**Próxima Sprint:** Nenhuma agendada — nenhum card do backlog original resta executável ou adiado. Próximo passo é um novo ciclo de auditoria futuro (recomendação permanente do CP05: a cada ~8-10 Sprints de feature/bugfix).
**Epic Atual:** Todos os 5 Epics (A-E) concluídos. Epic B: ARCH-005/006/007/008/010/011/012 feitos (ARCH-008 via reabertura), ARCH-009 encerrado (Won't Fix). Epic C: ARCH-013/014/015/016/017/018 todos feitos (ARCH-013/018 via reabertura).
**Próximo Marco:** nenhum agendado — o backlog original de 26 cards está inteiramente resolvido (concluído ou Won't Fix, nenhum adiado restante).
**Última Atualização:** 2026-07-19 (S18/ARCH-018 reaberta e concluída — último item do ciclo de auditoria pós-encerramento)
**Build:** 🟢 `npm run build` limpo
**Testes:** 🟢 131/131
**Regressão:** 🟢 131/131 (130 pós-ARCH-008 + `S128` novo — ARCH-018 — 0 quebras líquidas; `S30`/`S125.6` corrigidos por achado colateral direto)
**Riscos Abertos:** 1 materializado (não bloqueante, registrado no CP04, ainda válido): nenhum log de produção real está disponível para validar S24-S27 nem as reaberturas de S17/S18/S21 — a branch `refactor/architectural-backlog` continua não mesclada em `main` além do merge original (`f230fd9`/`v2.0.0`); toda evidência de comportamento real vem das etapas 4 (sandboxes isolados), não de observação de produção. Nota permanente: a VPS `venus@10.0.0.10` usada para validação Linux tem histórico de exigir `.env`/`WORKSPACE_DIR` explícito (skill `verify`) para não gerar falso-positivo — ver `docs/issues/002`. **Achado novo desta rodada (S17):** o canal Web Dashboard não tem `workflowCallback` wired para aprovar ações perigosas — registrado em `ARCHITECTURE.md`. **Achado novo desta rodada (S18):** um design aprovado na Fase 2/3 da diretriz (critério de bypass movido para `evaluateCriteria()`) tinha uma consequência real (replan desnecessário) só visível em ambiente real — nem o design nem os testes mockados a pegaram; corrigido e reconfirmado ao vivo. Reforça por que a etapa 4 é obrigatória, não dispensável, para mudanças nesta área.
...
**Temas Adiados sem Consolidação (categoria própria):** 0 — os 3 itens desta categoria (ARCH-008, ARCH-013, ARCH-018) foram todos implementados na reabertura.
...
**Dívida Arquitetural Restante:** 0 ARCH adiados, 0 executáveis — 26 total = 25 concluídos + 1 encerrado (Won't Fix). Nenhum candidato restante do backlog original. Próximo ciclo de auditoria é um evento futuro, não uma pendência atual.
**Achados fora de escopo documentados (`docs/issues/`):** 002 (falsos-positivos de validação VPS), 003 (`core/index.ts` barrel possivelmente morto), 004 (2 funções `extractText` com nome colidindo — candidato independente de quick win, não mais vinculado à revisão de tipos), 005 (regex de tool names desatualizada — corrigido de brinde na própria S10), 006 (análise de por que os 4 detectores de loop não convergem pra 1 fonte só — não é pendência, é registro de decisão), 008 (ARCH-009: `extends ToolResult` não compila + reintroduz violação de fronteira do ARCH-004 — motivo do adiamento original de S14, resolvido via Won't Fix), 009 (ARCH-008: "recovery no boot" citado pelo card não existe no runtime — motivo do adiamento original de S17, resolvido na reabertura via `buildInitialProgressModel`), 010 (ARCH-018: `file_exists` checa attempt não disco — motivo do adiamento original de S18, resolvido na reabertura via `pending_send_verified_on_disk`), 011 (ARCH-013: fundir sem ajuste perde sinal `stepSuccessConfident` — motivo do adiamento ORIGINAL de S21, resolvido na reabertura via `promoteLastAttemptToSuccess`), 012 (ARCH-009: auditoria de reabertura + decisão final Won't Fix, 2026-07-18)
