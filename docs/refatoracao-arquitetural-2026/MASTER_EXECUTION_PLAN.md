...
| 2026-08-S14 | Ago/2026 | Single Source of Truth | ARCH-009 | ⏸ | CP01 | 92c46da (docs) | Adiado antes de codificar — prescrição do card (`extends ToolResult`) não compila + reintroduz violação de fronteira do ARCH-004; consolidado em `REVISAO_CONSOLIDADA_TIPOS_PENDENTE.md` | [ver](SPRINTS/S14-ARCH-009.md) |
...
| 2026-08-S17 | Ago/2026 | Single Source of Truth | ARCH-008 | ⏸ | S16 | 4e01818 | Adiado antes de codificar — premissa citava "recovery no boot" que não existe (6º modo de falha); defeito real via resumeGoal()/auth, fix desenhado em `docs/issues/009` | [ver](SPRINTS/S17-ARCH-008.md) |
...
| 2026-09-S21 | Set/2026 | Decision Ownership | ARCH-013 | ⏸ | CP03, S14 | 505a8a9 | Adiado antes de codificar — premissa CORRETA e prescrição viável, mas fundir sem ajuste perde sinal `stepSuccessConfident` (7º modo de falha); fix desenhado em `docs/issues/011` | [ver](SPRINTS/S21-ARCH-013.md) |
...
**Status Geral:** 🟢 **Programa concluído** (2026-07-18) — Fase 0, S01-S27, Checkpoints CP01-CP05 todos concluídos; S14/S17/S18/S21 formalmente adiadas (ver abaixo). Todos os 26 cards do backlog têm desfecho final registrado.
**Progresso:** 22 / 26 ARCH concluídos (~85%) — ARCH-008 (S17), ARCH-009 (S14), ARCH-013 (S21) e ARCH-018 (S18) permanecem formalmente adiados (não contam como concluídos nem como pendências simples — são desfechos válidos e documentados). ARCH-024 concluído (RFC S19 + Impl S23). ARCH-020+ARCH-021 concluídos juntos (S24, ARCH-021 absorvido). ARCH-019 concluído (S25, 6 incrementos). ARCH-015 concluído (RFC S20 + Impl S26). ARCH-012 concluído (RFC + Impl de escopo reformulado, S27) — **nenhum card executável resta no backlog.**
**Sprint Atual:** Nenhuma em andamento (S27 concluída — último card executável do programa)
**Próxima Sprint:** Nenhuma — próximo passo é o Checkpoint de Encerramento `2026-12-CP05`.
**Epic Atual:** Epic A concluído; **Epic B concluído** — ARCH-005/006/007/010/011/012 todos feitos, ARCH-008 permanece adiado; **Epic C concluído** — ARCH-014/015/016/017 todos feitos (ARCH-013/018 adiados); **Epic D concluído** — ARCH-019/020/021/022/023/024 todos feitos; Epic E concluído — ARCH-025/026 feitos
**Próximo Marco:** 2026-12-CP05 (Checkpoint de Encerramento) — último marco do programa
**Última Atualização:** 2026-07-18 (Sprint S27 concluída)
**Build:** 🟢 `npm run build` limpo
**Testes:** 🟢 128/128
**Regressão:** 🟢 128/128 (pós-S27, 127 pré-existentes + `S125` novo, 0 quebras)
**Riscos Abertos:** 1 materializado (não bloqueante, registrado no CP04, ainda válido): nenhum log de produção real está disponível para validar S24-S27 — a branch `refactor/architectural-backlog` continua não mesclada em `main`; toda evidência de comportamento real vem das etapas 4 (sandboxes isolados), não de observação de produção — fechável só quando/se a branch for mesclada. Nota permanente: a VPS `venus@10.0.0.10` usada para validação Linux tem histórico de exigir `.env`/`WORKSPACE_DIR` explícito (skill `verify`) para não gerar falso-positivo — ver `docs/issues/002`. **S27 concluída com sucesso — o achado do CP04 (dependência ARCH-012→ARCH-018) foi resolvido na própria Fase 1 da RFC, não por reabrir ARCH-018**: a investigação mostrou que a dependência real era só para uma unificação de TIPO de dado (rejeitada por risco na Fase 2/4 da RFC) — o escopo efetivamente implementado (predicado puro compartilhado entre 3 mecanismos) não herda esse bloqueio. Achado mais valioso da Sprint: a premissa do card ("2 formas paralelas, nenhuma referencia a outra") não se sustentava — havia 3 mecanismos, e o problema real era divergência de RIGOR entre eles (2 de 3 não verificavam tipo de arquivo), não duplicação — mesmo padrão de correção de premissa já visto 7+ vezes neste programa (S08/S10/S11/S14/S16/S17/S18/S21).
...
**Temas Adiados sem Consolidação (categoria própria):** 3 (ARCH-008 — premissa citava mecanismo de recovery-no-boot inexistente, `docs/issues/009`; ARCH-018 — `file_exists` não faz o que o card presumia, reaproveitá-lo reintroduziria o deadlock histórico, `docs/issues/010` — **dependência de ARCH-012 resolvida em S27 sem reabrir este item** (ver card `ARCH-012`); ARCH-013 — premissa correta, mas fundir sem ajuste perde o sinal `stepSuccessConfident`, `docs/issues/011`, categoria nova — 7º modo de falha)
**Dívida Arquitetural Restante:** 4 ARCH, todos formalmente adiados, 0 executáveis (ARCH-008/S17, ARCH-009/S14, ARCH-013/S21, ARCH-018/S18) — 26 total − 22 concluídos = 4. Candidatos naturais ao próximo ciclo de auditoria (ver Checkpoint CP05).
