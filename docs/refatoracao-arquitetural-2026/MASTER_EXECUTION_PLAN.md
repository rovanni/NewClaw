...
**Status Geral:** 🟢 Em execução — Fase 0, S01-S13/S15/S16/S19/S20/S23/S24/S25, Checkpoints CP01/CP02/CP03 concluídos; S14/S17/S18/S21 adiadas (ver abaixo)
**Progresso:** 20 / 26 ARCH concluídos (~77%) — ARCH-008 (S17), ARCH-009 (S14), ARCH-013 (S21) e ARCH-018 (S18) adiados, não contam como concluídos nem como pendências simples. ARCH-024 concluído (RFC S19 + Impl S23). ARCH-020+ARCH-021 concluídos juntos (S24, ARCH-021 absorvido). ARCH-019 concluído (S25, 6 incrementos). ARCH-015 tem RFC aprovada (S20), só conta como concluído quando a Impl (S26) landar
**Sprint Atual:** Nenhuma em andamento (S25 concluída — próxima é 2026-11-S26)
**Próxima Sprint:** 2026-11-S26 (ARCH-015-Impl) — implementar `requiredArgsHint` (escopo já reduzido pela RFC aprovada em S20: só texto de prompt co-localizado por tool, sem a metade de validação condicional). Risco baixo/médio — escopo pequeno, já definido pela RFC, sem necessidade do padrão de incrementos usado em S24/S25.
**Epic Atual:** Epic A concluído; Epic B "concluído" no sentido de nenhum item executável restante nesta ordem — ARCH-005/006/007/010/011 feitos, ARCH-008 adiado, ARCH-012 deliberadamente deferido para o fim do programa; Epic C — ARCH-014/015(RFC)/016/017 feitos/aprovados, ARCH-013/018 adiados; **Epic D concluído** — ARCH-019/020/021/022/023/024 todos feitos; Epic E concluído — ARCH-025/026 feitos
**Próximo Marco:** 2026-11-CP04
**Última Atualização:** 2026-07-18 (Sprint S25 concluída)
**Build:** 🟢 `npm run build` limpo
**Testes:** 🟢 126/126
**Regressão:** 🟢 126/126 (pós-S25, 0 quebras de teste em nenhum dos 6 incrementos)
**Riscos Abertos:** 0 materializados — nota permanente: a VPS `venus@10.0.0.10` usada para validação Linux tem histórico de exigir `.env`/`WORKSPACE_DIR` explícito (skill `verify`) para não gerar falso-positivo — ver `docs/issues/002`. **S25 concluída com sucesso — o maior refactor do programa (AgentLoop.runWithTools(), ~1.7x o tamanho de S24/GoalExecutionLoop.runLoopInternal(), ~3x mais variáveis mutáveis por closure).** Risco mitigado pelo mesmo padrão validado em S24 (incrementos sequenciais com etapa 4 própria cada), escalado de 2 para 6 incrementos em ordem crescente de risco — decisão do usuário, não minha, tomada explicitamente em resposta ao tamanho maior. 3 classes DISTINTAS de erro de referência/tipo/parâmetro foram pegas por `tsc` (não por teste) em 3 incrementos diferentes (4, 5, 6) — nenhuma da mesma classe dos bugs de VALOR (threading de `priorFeedback`) que a leitura cuidadosa pegou em S24. Confirma que os dois mecanismos (verificação de tipo automática + revisão humana/LLM linha a linha) são complementares: nenhum sozinho cobre as duas classes de erro. Um método (`runWithTools()`, o orquestrador remanescente) ficou acima do limite de 300 linhas (419) mesmo após 2 cortes — aceito como exceção documentada pelo usuário, não por decisão minha unilateral: o que resta é o esqueleto de controle do `while` loop + declarações que precisam ficar antes do `try{}` (achado real: mover a criação de `trace` para dentro de uma fase extraída teria quebrado a semântica do `catch(fsmError)`) + o `catch/finally` obrigatório.
**RFCs Pendentes:** 1 (ARCH-012, deliberadamente por último) — ARCH-024 (S19+S23, concluído) e ARCH-015 (S20, RFC aprovada) — Impl de ARCH-015 agendada para S26
...
**Dívida Arquitetural Restante:** 6 ARCH (ARCH-015-Impl/S26, ARCH-012/S27 — 2 cards executáveis restantes + ARCH-008/ARCH-009/ARCH-013/ARCH-018 adiados) — 26 total − 20 concluídos (S25 fechou ARCH-019) = 6
