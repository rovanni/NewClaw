...
- `SessionTranscript.migrateLegacyColonPath` (`project_session_bugs_jul2026_am`, memória): passou em teste de regressão e `tsc --noEmit`, mas falhava de verdade in runtime (`fs.renameSync` retorna `EINVAL` ao mover de uma NTFS Alternate Data Stream no Windows) — só descoberto rodando contra uma cópia de dado real.
