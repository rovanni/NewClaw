...
// ── scaffolding mínimo (mesmo padrão de S158/S84: loop real, LLM/planner/memória fake) ─────────

function fakeProviderFactory(validationJson: object) {
...
function makeLoop(validationJson: object) {
...
function makeGoal(store: GoalStore, plan: PlanStep[], over: Partial<Goal> = {}): Goal {
    return store.create({
        sessionKey: 'test:s278', conversationId: `s278-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
...
const emptyState = (goalId: string) => ({
